...
# Distance matrix (fill in with your actual distance values if available)
...
# Define the parameters for the cost calculation (fill in with appropriate values)
m <- runif(n, min = 100000, max = 500000)
d <- runif(n, min = 10, max = 100)
c <- matrix(runif(n*n, min = 0.1, max = 10), ncol = n)

# Define the remaining variables (fill in with appropriate values)
alfa <- runif(1, min = 0.1, max = 1)
h <- runif(1, min = 0.1, max = 1)
beta <- runif(n, min = 0, max = 1)
gamma <- runif(n, min = 0, max = 1)
pc <- array(runif(n*n*T, min = 0, max = 1), dim = c(n, n, T))

jumlah_permintaan <- runif(n, min = 5000, max = 20000)
kapasitas_kapal <- 50
kapasitas_port <- runif(n, min = 1000000, max = 5000000)
kapasitas_gudang <- 100
jumlah_waktu_sampai <- 10
jumlah_kapal <- 3

# Membuat variabel untuk setiap port
port_A <- c(5, 6)  # Container kosong dan container penuh dari port A
port_B <- c(6, 2)  # Container kosong dan container penuh dari port B
port_C <- c(8, 10)  # Container kosong dan container penuh dari port C
port_D <- c(5, 4)  # Container kosong dan container penuh dari port D
...
model <- MIPModel() %>%
  # Variables
  add_variable(x[i, j, t], i = 1:n, j = 1:n, t = 1:T,
               type = "binary") %>%
  # a helper variable for the MTZ formulation of the tsp
  add_variable(u[i], i = 1:n, lb = 1, ub = n) %>%
  # Objective function: minimize travel distance
  set_objective(sum_expr(distance[i, j] * x[i, j, t], i = 1:n, j = 1:n, t = 1:T), "min") %>%
  # Constraints
  # Leave each city
  add_constraint(sum_expr(x[i, j, t], j = 1:n) == 1, i = 1:n, t = 1:T) %>%
  # Visit each city
  add_constraint(sum_expr(x[i, j, t], i = 1:n) == 1, j = 1:n, t = 1:T) %>%
  # Ensure no subtours (arc constraints)
  add_constraint(u[i, t] >= 2, i = 2:n, t = 1:T) %>%
  add_constraint(u[i, t] - u[j, t] + n * x[i, j, t] <= n - 1, i = 2:n, j = 2:n, t = 1:T) %>%
